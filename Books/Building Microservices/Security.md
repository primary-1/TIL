...
### 백업 암호화
    ...
    백업은 좋은 것이다. 금덩이라고 나는 생각한다. 백업 과정 자체가 존재하지 않는다면, 후에 재난상황 후 가장 먼저 찾을 것이다.
    중요한 데이터를 백업하고 싶어 하며 너무 민감해서 암호화가 필요한 데이터는 백업도 매우 중요하다고 정의할 수 있다.

중요한 데이터를 암호화 한 데이터도 백업을 해야 한다, 그 데이터 또한 암호화 하였는지 확인해야한다.

이렇게 백업을 하기 전에는 암호화를 항상 하는데 그러므로 **명확한 키 관리는 매우 중요하다.**

심층 방어
-----------

    모든 곳에 정보를 모아두는 것은 굉장히 취약한 행동이다.
    따라서 심층 방어가 최선인데, 전송 중인 데이터와 보관 중인 데이터의 보안을 쉽게 할 수 있는 방법은 없을까?

### 방화벽

    일반 방화벽은 단순해서 특정 포트에 대해 특정 종류의 트래픽 접근을 제한하는 것만 가능하지만 어떤 것은 보다 정교하다.

예를 들어 아파치의 오픈소스 방화벽인 ModSecurity는 애플리케이션 방화벽의 한 종류로 특정 IP 대역에서 유입되는 접속을 차단하도록 조절할 수 있고, 다른 종류의 악의적 공격을 탐지할 수 있다.

#### 다중 방화벽
    하나 이상의 방화벽을 구축하는 것은 취할 수 있는 매우 현명한 예방 조치다.
    호스트의 보안을 위해 허용 가능한 진입 및 진출 트래픽 설정 같은 IPTable을 사용하여 다중으로 된 방화벽을 구축할 수 있다.

> 이들 규칙은 일반적인 접근 통제를 위한 네트워크 경계에 있는 방화벽과 함께 호스트 내에서 실행되는 서비스를 대상으로도 설정 할 수 있다.

### IDS, IPS

    IDS(intrusion detection system)는 문제가 발견될 때 리포팅하며 수상한 행위에 대해 네트워크와 호스트를 모니터링할 수 있다.
    IPS(intrusion prevention system)은 수상한 활동을 모니터링 할 뿐만 아니라 그것이 발생되지 않도록 한다.

나쁜 것이 유입되는 것을 막기 위해 외부 지향적인 방화벽과는 달리 IDS와 IPS는 적극적으로 경계 내부의 수상한 행위를 관찰한다.

문제 경보 면에서 수동적인 IDS를 사용하면 그 규칙들을 더 적극적으로 적용하기 전에 잘 조율한 기회를 얻게 된다.

### 망 분리

    모놀리식 서비스는 모든 서비스가 하나로 하져 있어 추가 보호를 위해 네트워크를 구성할 수 있는 방법이 제한되어있지만
    마이크로 서비스에서는 서비스들의 상호 통신 방법의 통제를 세분화하도록 서비스들을 다른 네트워크 세그먼트에 배치할 수 있다.

[AWS의 VPC(Virtual Private Cloud)는 자동적으로 프로비저닝하는 기능을 제공한다.](https://docs.aws.amazon.com/ko_kr/AmazonVPC/latest/UserGuide/VPC_Introduction.html)

그래서 상호 접속 규칙을 정의해 VPC 간의 가시성을 명시 할 수 있고, 추가 보안 조치가 적용될 수 있는 다수의 네트워크 경계를 효율적으로
제공하면서 게이트웨이를 통해 트래픽이 프록시로 오도록 라우팅도 가능하다.

### 운영 체제

    우리의 시스템은 우리가 작성하지 않은 소프트웨어와 여러 통신을 하고 있다.
    그렇기에 만약이라도 운영체제 혹은 도구 같은 것의 취약점이 발견된다면, 해당 소프트웨어는 의도치 않게 결점이 생길 것이다.

그렇기에 몇 가지 예방 방안이 존재한다.

1. 계정이 누출되더라도 피래를 최소화할 수 있도록 가능한 적은 권한을 가진 OS 사용자로 동작하는 서비스로 시작하라.

2. 소프트웨어를 정기적으로 패치하라.
    - 패치는 정말로 기본적인 것이지만 패치가 안 된 오래된 운영 체제에서 매우 중대한 소프트웨어가 실행되는 것을 자주 보면 놀라울 따름이다.
    - BOF 취약점이 패치되지 않은 구 버전의 root권한으로 실행되고 있다면 그 시스템은 극도로 취약할 것이다.

3. 리눅스를 사용 중이라면 보안 모듈을 살펴보라.
    - 앱아머(AppArmour)는 커널을 감시하면서 해플리에키션에 기대되는 행동 양식을 정의하게 한다. (금지된 행위를 할 경우 커널이 개입한다)

절약하라
-----------

    디스크 공간 비용이 낮아지고 데이터베이스 기능이 향상됨에 따라 대용량 정보의 수집과 저장이 급격히 쉬워지고 있다.
    이 데이터는 그것을 점차 소중한 자산으로 여기는 기업뿐만 아니라 자신의 프라이버시를 소중히 여기는 사용자에게도 똑같이 소중하다.

개인 정보 데이터 혹은 개인 정보를 알아낼 때 사용하는 데이터는 우리가 가장 신경 써야 할 데이터다.

이 개인 정보 데이터를 개인을 식별할 수 있는 정보를 가능한 제거해서 적용한다면 여러 이점이 있다.

1. 저장하지 않는다면 훔쳐갈 것도 없다.
2. 저장하지 않는다면 아무도 달라고 요청할 것도 없다.

인적 요소
-----------

    기술적인 문제 외에 인적 요소 또한 중요하다, 만약에 퇴사한 직원이 악의적인 목적으로 시스템에 접근한다면? 혹은 사회 공학으로부터 어떻게 보호할 것인지?

불팜을 품은 퇴사한 직원이 마음만 먹는다면 시스템에 어떤 피해를 줄 수 있는지 고민해보라.
악의가 있는 사람의 입장에서 생각하는 것은 필요한 보호 조치를 추론할 수 있는 좋은 방법이며, 그중 일부는 이미 현재의 직원만큼 많은 내부 정보를 보유하고 있다!
