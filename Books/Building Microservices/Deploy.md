...
분리된 서비스나 애플리케이션이 하나의 호스트 상의 에플리케이션 컨테이너에 머무르는 모델에 대해서도 잘 알고 있을 것이다. 애플리케이션 컨테이너 안에 서비스가 머무르는 아이디어는 인스턴스들의 그룹화나 모니터링 도구 등을 다루는 클러스터링 지원처럼 관리성이 향상된 측면에서 많은 혜택을 가져다준다.>???
